...
class FlightCardUnclickable(FlightCard):
    ...
    def build(self):
        if self.flight.departure_time is None:
            self.card = Container(
                content=Row(
                    [
                        ...
                        Column(
                            [
                                ...
                                Text(f"{self.flight.arrival_time}", size=16,
        ...
        elif self.flight.arrival_time is None:
            self.card = Container(
                content=Row(
                    [
                        # Left Section: Outbound, Destination, Time
                        Column(
                            [
                                Text(f"{self.flight.origin} → {self.flight.destination}",
                                     size=20, weight=FontWeight.BOLD),
                                Text(f"{self.flight.departure_time}", size=16,
                                     weight=FontWeight.BOLD),
                                # Text(self.flight.departure_date, size=16, weight=FontWeight.BOLD),
                            ],
                            alignment=MainAxisAlignment.START,
